...
function ModifyStagestagiaire ()
{
    ...
    function handleChangesingle(selectedOption)
    {
       ...
       axios.get(`http://localhost:8000/api/stagestagiaire/${selectedOption.value.id}/`)
        .then(res => {
          console.log("id:",res.data.id);
          console.log("data",res.data);
          console.log("Agreement:",res.data.PDF_Agreement);
          year={
            value:res.data.Annee,
            label:`${res.data.Annee}`,
          }
          setUniversite(res.data.Universite);
        setAnnee(year);
        x=res.data.Annee_etude.split('_');
        console.log("x:",x);
        colyear={
          value:x[0],
          label:res.data.Annee_etude,
        }
        ...
        if(res.data.PDF_Agreement!=null)
        {
          setPDF_Agreement(res.data.PDF_Agreement);
          agreementpath=res.data.PDF_Agreement.split('/');
        ...
        if(res.data.Rapport!==null)
        {
          setrapport(res.data.Rapport);
          rapportpath=res.data.Rapport.split('/');
        ...
        if(res.data.Presentation!==null)
        {
          setpresentation(res.data.Presentation);
          presentationpath=res.data.Presentation.split('/');
        ...
        if(res.data.PDF_Prolongement!==null)
        {
          setPDF_Prolongement(res.data.PDF_Prolongement);
          prolongementpath=res.data.PDF_Prolongement.split('/');
          setprolongementsliced(prolongementpath[(prolongementpath.length)-1]);
        }
        setprgbrsval(0);
        if(res.data.PDF_Certificate!==null)
        {
          setPDF_Certificate(res.data.PDF_Certificate);
          certificatepath=res.data.PDF_Certificate.split('/');
              ...
              if((certificatefile!==null && codefile!==null && rapportfile!==null && presentationfile!==null)||(formData.PDF_Certificate!==null && formData.Code!==null && formData.Rapport!==null && formData.Presentation!==null))
                {
                  ...
                  if(PDF_Certificate===null)
                  {
                    ...
                    form.append('N_stage',numberstages+1);
                    axios.patch(`http://localhost:8000/api/Stagiaires/${parseInt(singleselectedoption.value.value)}/`,form,{
                      headers: {
